...
class Attention_org2(nn.Module):
    def __init__(self, config, vis,channel_num):
        super(Attention_org2, self).__init__()
        self.vis = vis
        self.KV_size = config.KV_size
        self.channel_num = channel_num
        self.num_attention_heads = config.transformer["num_heads"]
    ...
    def forward(self, emb1,emb2,emb3,emb4, emb_all,txt):
        multi_head_Q1_list = []
        multi_head_Q2_list = []
        multi_head_Q3_list = []
        multi_head_Q4_list = []
...
class Block_ViT(nn.Module):
    ...
    def forward(self, emb1,emb2,emb3,emb4,txt):
        embcat = []
        org1 = emb1
        org2 = emb2
        org3 = emb3
...
class Encoder(nn.Module):
    ...
    def forward(self, emb1,emb2,emb3,emb4,txt):
        ...
        for layer_block in self.layer:
            emb1,emb2,emb3,emb4, weights = layer_block(emb1,emb2,emb3,emb4,txt)
            if self.vis:
...
class ChannelTransformer(nn.Module):
    ...
    def forward(self,en1,en2,en3,en4,txt):
        ...
        encoded1, encoded2, encoded3, encoded4, attn_weights = self.encoder(emb1,emb2,emb3,emb4,txt)  # (B, n_patch, hidden)
